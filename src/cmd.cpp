...
uint16_t pico_cmd_get(uint8_t report_id, uint8_t *buffer, uint16_t reqlen) {
    if (report_id == 0xf7) {
        ...
        // [REMAP-DISABLED] OLED Edition remap block: button remapping disabled,
        // so 0xF7 returns config only (no 'R''M' remap payload appended).
...
void pico_cmd_set(uint8_t report_id, uint8_t const *buffer, uint16_t bufsize) {
    ...
    // [REMAP-DISABLED] 0x10 set button-remap table: button remapping disabled,
    // so the 0x10 func-id is rejected (falls through, no handler).
    //   [0]      0x10  func-id
    //   [1]      'R'
    //   [2]      'M'
