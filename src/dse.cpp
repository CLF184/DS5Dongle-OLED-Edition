...
//
// 上游 8a2f576 原文移植。唯一改动：fork 的 feature_data 约定是"带报告 ID"
// （assign(packet+1) / memcpy(data+1)），所以 0x20 固件报告取 fw.data() + 1；
// 上游 2efc855 改成不带 ID 后代码是 fw.data()。
//
...
void dse_on_connect() {
    ...
    if (cid != 0 && it != feature_data.end() && it->second.size() >= 62) {
        ...
        handshake[0] = 0x53;
        handshake[1] = 0x65;
        memcpy(handshake + 2, fw.data() + 1, 61); // fork 约定：跳过报告 ID
